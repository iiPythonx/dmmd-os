(async () => {
    ...
    const settings = {
        ...
        skip_bios:  iframe ? false : +localStorage.getItem("skip_bios"),
        skip_boot:  iframe ? false : +localStorage.getItem("skip_boot"),
        live_clock: iframe ? true  :  localStorage.getItem("live_clock")    !== "0",
        boot_id:    iframe ? true  :  localStorage.getItem("show_boot_id")  !== "0",
    };
    const bios = {
        boot: () => {
        ...
        load: () => {
            if (settings.skip_load) return bios.boot();
            document.querySelector("body").innerHTML = `
                <h1>iiPython OS</h1>
                <div>
        ...
        init: async () => {
            if (settings.skip_boot) return bios.boot();
            if (settings.skip_bios) return bios.load();
            ...
            function dump() {
                ...
                if (current.index !== current.items.length) setTimeout(dump, 200);
                    ...
                    if (!settings.boot_id) document.querySelector("footer > span:last-child").remove();
